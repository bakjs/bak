...
class Server {
    ...
    route(routes) {
        return new Promise((resolve, reject) => {
            ...
            // Workaround: hapi.route() has not any callback :(
            let _route_counter = 0;
            let _resolved = false;
            this.hapi.on('route', () => {
                if (++_route_counter < routes.length) return;
                if (!_resolved) return resolve();
            });
