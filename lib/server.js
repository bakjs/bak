...
module.exports = class Server {
  ...
  async _loadRoutes (routes, dirty = false) {
    if (typeof routes === 'string') {
      dirty = true
      routes = this.bak.require(routes)
    }
    ...
    if (routes.init instanceof Function) {
  ...
  register (registration, registrationOptions) {
    ...
    const configKey = camelCase(registration.plugin.configKey || registration.plugin.name || (registration.plugin.pkg && registration.plugin.pkg.name) || '')

    if (configKey.length && this.options[configKey]) {
