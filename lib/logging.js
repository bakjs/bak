...
const LoggingPlugin = {
    register: function (server, config, next) {
        ...
        // Register this hook to print routes table
        if (process.env.NODE_ENV !== 'production')
            register_route_logger(server, config);

        // Register good for requests log
        register_good(server, config);

        if (next) next();
    }
};

function register_good(server, config, next) {
    // TODO: less logs for NODE_ENV=production
    server.register({
...
function register_route_logger(server, config, next) {
