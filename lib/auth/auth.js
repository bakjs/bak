...
class Auth {
    ...
    expose(server) {
        ...
        server.decorate('request', 'get_token', function get_token(user, client) {
            return self.get_token(user, this, client);
        });
    }
    ...
    async get_token(user, request = {headers: [], info: []}, client) {
        ...
        if (client) {
            session._client = client._id || client;
        }

        // Revoke any older sessions of user with same user-agent and ip
        if (this.auth_options.auto_logout === true) {
            user.sessions = user.sessions.filter(s => (s._agent !== session._agent || s._ip !== session._ip));
        }
    ...
    async oauth_authorize(client_id, request) {
        ...
        // Get user profile
        let {user} = await Axios.get(`${client.url}/user`, {
