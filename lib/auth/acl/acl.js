...
exports.checkRoles = function (user, role, hierarchy) {
    ...
    if ((!user) || (!internals.isGranted(user.roles, role, hierarchy))) {
...
internals.isGranted = function (userRole, requiredRole, hierarchy) {
    ...
    if (hierarchy) {
        let index = hierarchy.indexOf(userRole);	// Get the index of userRole in the hierarchy
...
exports.validateEntityAcl = function (user, role, entity, validator, options) {
    ...
    if (!entity) {
        ...
        if (validator) {
            ...
            let isValid = internals.defaultEntityAclValidator(user, role, entity, options);
