    ...
    const authenticate = (request, reply) => {
        try {
            ...
            jwt_verify(token, auth_options.secret).then((token_decoded) => {
                ...
                if (!token_decoded) return reply(Boom.unauthorized('INVALID_TOKEN'));
                ...
                auth_options.user_model.findById(user_id).then((user) => {
                    ...
                    if (!session) {
                        return reply(Boom.unauthorized("SESSION_EXPIRED"));
                    }
                    reply.continue({
                        credentials: user,
                        artifacts: session,
                    ...
                    return reply(Boom.unauthorized('USER_NOT_FOUND'));
                });

            }).catch(() => {
                return reply(Boom.unauthorized('MANIPULATED_TOKEN'));
            });
        } catch (e) {
            reply(Boom.unauthorized("AUTH_ERROR"));
        }
    };
