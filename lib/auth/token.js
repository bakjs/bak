...
const plugin = (server, auth_options) => {
    ...
    const defaultValidate = (token, session, user) => {
        return jwt_verify(token, auth_options.secret);
    };

    // Prefer user provided validate
    const validate = auth_options.validate || defaultValidate;

    // Default User auth
    const defaultAuth = async (token) => {
        ...
        // Validate token
        let validated = await validate(token, session, user);
    ...
    // Prefer user provided auth
    const auth = auth_options.auth || defaultAuth;
