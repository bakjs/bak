...
//const RedisRateLimit = require('./redis');
const MemoryRateLimit = require('./memory');
...
function HapiRateLimit(plugin, _options, next) {
    ...
    const limiter = new MemoryRateLimit(options);

    const handleLimits = (request, reply) => {
        const route = request.route;

        // Get route-specific limits
        let routeLimit = route.plugins && route.plugins.ratelimit;
