...
module.exports = class Controller {
  ...
  _internal_error (error, route, request, reply) {
    ...
    if (error && error.isBoom) {
      if (reply && !reply._replied) { reply(error) }
      return
    }
    ...
    const reply_error = (ref) => {
      if (reply && !reply_error_called && !reply._replied) { reply({ error: 'Internal server error', statusCode: 500, ref }).code(500) }
    ...
    if (this.hapi) { this.hapi.log(tags, context) } else console.error(context)
  }
  ...
  _resolve_handler_params (request, reply, route_params) {
    ...
    route_params.forEach(({ param_id, is_optional }) => {
      ...
      if (routeKey === '_id' && !Mongoose.Types.ObjectId.isValid(request_param)) {
        if (!reply._replied) { reply(Boom.badRequest('invalid id for ' + param_id)) }
        return null
      }
