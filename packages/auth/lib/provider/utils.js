...
function hash_verify (password, _hash) {
  ...
  switch (hash[1]) {
    ...
    default:
      return Promise.resolve(password === hash[0])
  }
}
...
function uid (length) {
  return new Promise((resolve, reject) => {
    Crypto.randomBytes(length * 2, (err, bytes) => {
      if (err) return reject(err)
      resolve(bytes.toString('base64').replace(/\+/g, '').replace(/\//g, '').replace(/=/g, '').substring(0, length))
    })
  })
}
